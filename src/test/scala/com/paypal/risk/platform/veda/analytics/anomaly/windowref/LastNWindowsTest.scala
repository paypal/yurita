...
class LastNWindowsTest extends FlatSpec with Matchers {

  behavior of "LastNWindows"

  it should "return the previous window Key" in {
    val ref = new LastNWindows(1)
    val refFunc = ref.getReferenceWindowKeysFunc(Window.fixed("10 seconds"))
    refFunc("4446") shouldBe Seq("4445")
  }

  it should "return nth previous windows key" in {
    val ref = new LastNWindows(2)
    val refFunc = ref.getReferenceWindowKeysFunc(Window.fixed("10 seconds"))
    refFunc("4446") shouldBe Seq("4444", "4445")
  }
