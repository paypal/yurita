...
/** defines how to aggregate a given metric into windows */
trait WindowStrategy extends Serializable {

  /** extract from a timestamp its relevant window keys for grouping */
  def extractTimeWindowKeys: java.sql.Timestamp => Seq[String]

  /** for a given window key return a timestamp in milliseconds specifying when this window ends */
  def getTimeoutTimestamp: String => Option[Long]

  /** for a given window key return a timestamp in milliseconds specifying when this window start.
   * this is optional and used mostly for logging
   *
   */
  def getStartTimestamp: String => Option[Long] = _ => None
}
